...
必ず有効なJSONオブジェクトのみを返却し、マークダウンの \`\`\`json などのタグも使用しないでください。`;
    ...
    const response = await client.models.generateContent({
      ...
      config: {
        ...
        responseSchema: {
          type: Type.OBJECT,
          required: ["translation", "level", "options", "sentence", "sentenceTranslation", "sentenceOptions"],
          properties: {
    ...
    return res.json({
      ...
      frequencies: {
        everyday: {
          ...
          label: everydayScore >= 4 ? "高い" : (everydayScore >= 3 ? "普通" : "低い"),
          description: `日常的な日常会話で使用される頻度。${everydayScore >= 4 ? "基本的で使い勝手が良く、話し言葉として非常によく耳にする語彙です。" : "会話ではもっと平易で単純な動詞・形容詞が好まれる傾向にあります。"}`
        },
        academic: {
          score: academicScore,
          percentage: academicScore * 20,
          label: academicScore >= 4 ? "高い" : (academicScore >= 3 ? "普通" : "低い"),
          description: `学術文献や講義、レポートでの頻度。${academicScore >= 4 ? "客観性や論理性を重んじる学術文章やプレゼンに欠かせない、フォーマル性の高い表現です。" : "一般名詞寄りのため、正確無比な科学論文などではより専門的（テクニカル）な表現が好まれます。"}`
        },
        business: {
          score: businessScore,
          percentage: businessScore * 20,
          label: businessScore >= 4 ? "高い" : (businessScore >= 3 ? "普通" : "低い"),
          description: `ビジネスメール、交渉、経営会議での頻度。${businessScore >= 4 ? "ビジネスプランの構築やチーム内の意思決定プロセス、顧客交渉などにおいて最瀕出するワードです。" : "カジュアルさが残るため、厳格な英文契約書や経営報告書では別の改まった表現が選ばれることがあります。"}`
        }
      },
      overallComment: `この単語「${word.trim()}」は、全体的に${academicScore >= 4 ? "フォーマルな要素" : "実用的でシンプルな性質"}を強く帯びています。シチュエーションに応じた適切な置き換えができるとワンランク上の英語使いになれます。`,
      usageExamples: {
        everyday: {
          sentence: `I'll try to find a natural way to use "${word.trim()}" in daily chat.`,
          translation: `日常会話の中で「${word.trim()}」を自然に使う方法を探してみるよ。`
        },
        academic: {
          sentence: `This study focuses primarily on the analytical factors surrounding "${word.trim()}".`,
          translation: `この研究は主に「${word.trim()}」を取り巻く分析的要因に焦点を当てています。`
        },
        business: {
          sentence: `We need to analyze how we can leverage "${word.trim()}" in our operations.`,
          translation: `私たちは業務の中で「${word.trim()}」をいかに活用できるかを分析する必要があります。`
        }
      }
    });
  }
...
注意事項: マークダウンの \`\`\`json タグなどを一切付加せず、純粋なJSONオブジェクトのみを返却してください。`;
    ...
    console.warn("単語頻度分析に失敗したため、ローカルフォールバックモードで動作します。");
    const len = queryWord.length;
    const isAcademic = len > 8 || queryWord.endsWith("tion") || queryWord.endsWith("ity") || queryWord.endsWith("ive") || queryWord.includes("struct");
    const isBusiness = queryWord.includes("act") || queryWord.includes("project") || queryWord.includes("strategy") || queryWord.includes("meet") || queryWord.includes("sign") || queryWord.includes("serve") || queryWord.includes("offer") || queryWord.includes("press");
    ...
    return res.json({
      ...
      overallComment: `この単語「${word.trim()}」は、全体的に${academicScore >= 4 ? "フォーマルな要素" : "実用的でシンプルな性質"}を強く帯びています。シチュエーションに応じた適切な置き換えができるとワンランク上の英語使いになれます。(一時的なFallback表示中)`,
      usageExamples: {
        everyday: {
          sentence: `I'll try to find a natural way to use "${word.trim()}" in daily chat.`,
          translation: `日常会話の中で「${word.trim()}」を自然に使う方法を探してみるよ。`
