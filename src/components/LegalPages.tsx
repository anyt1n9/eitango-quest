...
export function PrivacyPolicy({ onBack }: { onBack: () => void }) {
  return (
    <Shell title="プライバシーポリシー" icon={<ShieldCheck className="w-6 h-6" />} onBack={onBack}>
      ...
      <h2>2-2. Googleドライブへの保存（任意）</h2>
      <p>
        「データ」の画面でGoogleドライブとの同期を選んだ場合に限り、上記の学習データが
        <strong>利用者ご自身のGoogleドライブ</strong>（本アプリ専用の隠しフォルダ）に保存されます。
        保存先は利用者のドライブであり、<strong>当方のサーバーには送信も保存もされません</strong>。
        本アプリが求める権限は「このアプリが作成したファイルの読み書き」のみで、
        ドライブ内の他のファイルを読むことはできません。同期は自動では行われず、
        利用者が「ドライブへ保存」「ドライブから復元」を押したときだけ実行されます。
        連携をやめたいときは、Googleアカウントの
        {" "}<a href="https://myaccount.google.com/permissions" target="_blank" rel="noopener noreferrer">アプリの権限</a>
        {" "}から本アプリのアクセスを取り消せます。
      </p>
...
export function AboutApp({ onBack }: { onBack: () => void }) {
  return (
    ...
    >
      ...
      <h2>6. データの保存</h2>
      <p>
        学習の記録はすべて、お使いの端末（ブラウザ）の中だけに保存されます。当方のサーバーには送られません。
        端末を変えるときや消えてしまうのが心配なときは、2つのやり方から選べます。
      </p>
      <ul>
        <li>
          <strong>ファイルに書き出す</strong> … 「データ」の画面から書き出して、別の端末で読み込みます。
          アカウントは要りません。
        </li>
        <li>
          <strong>Googleドライブに置く</strong> … ご自身のドライブの、このアプリ専用の隠しフォルダに保存します。
          別の端末で同じGoogleアカウントにログインすれば、続きから学習できます。
          こちらもデータが当方のサーバーを通ることはありません。
        </li>
      </ul>
      <p>
