...
export const NON_BACKUP_KEYS: Record<string, string> = {
  ...
  // カードとボタンの透明感。画面の大きさや明るさで読みやすさが変わるので、
  // 端末ごとに選び直すもの（背景の設定と同じ扱い）
  quest_glass: "端末ごとの見た目の設定のため",
  // この端末でドライブ同期を使ったかどうかの印。
  // 復元先の端末はまだ許可していないので、持っていくと
  // 「開いただけでGoogleへ問い合わせる」ことになる
  quest_drive_linked: "端末ごとの連携状態で、移すと未許可の端末から通信が出るため",
};
