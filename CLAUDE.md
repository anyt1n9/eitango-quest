...
**カードとボタンをガラスにする**こともできる（`quest_glass`。オフ／うすく／しっかり／透明）。
既定は**オフ（不透明）**で、選んだ人にだけ効く。対象は「ボタン一覧の台」と
「レベルのカード」（`.glass-panel`）と、その中の塗りつぶしボタン（`.glass-chip`）。

- 濃さは `:root[data-glass="..."]` の変数（`--glass-alpha` `--glass-chip-alpha` `--glass-blur`）。
  値は**実測で決めた**もので、目分量で動かさないこと。
- **ここでは `backdrop-filter` を使う。** 背景写真にかける幕（`.bg-photo-veil`）で
  ぼかしを避けたのは、画面全体がモザイクに見えるため。カードの背後をぼかすのは
  逆に文字を浮き立たせるので、目的が違う。
  なお `-webkit-` は**自分で併記しない**。併記するとビルドが prefix 付きだけを残し、
  Chrome / Firefox でぼかしが効かなくなる（実測。computed 値が `none` になる）。
- 透けると沈む2つを、ガラスのときだけ補強している。
  補助の灰色（`text-gray-400` `text-gray-500`）は不透明な地でようやく基準を満たす濃さなので、
  少し透かすだけで下回る。海（暗いテーマ）の淡色（`--lv-bg`）は半透明なので、
  カードの色と混ぜて不透明にする。森の淡色はもともと不透明なので**触らない**
  （混ぜ直すと地が濃くなり、濃い文字とのコントラストがかえって下がる。実測 4.05）。
- レベルの色の変数は要素自身（`.level-*`）に置かれるので、
  カード側に書いても上書きできない。`.glass-panel [class*="level-"]` で当てる。

**「透明」（`clear`）だけは保証の外**にある。地をまったく置かないので、文字の地は
背景の写真そのものになり、どんな色が来るかを決められない。実測（描かれた画素で測ったもの）で
明るい写真の上 1.73、暗い写真の上 1.14 まで落ちる。**ここは薄くする余地の問題ではなく、
固定の文字色では原理的に保証できない**（保証するには地の明るさを狭い範囲に押し込むしかなく、
それは結局ほぼ不透明な膜と同じになる。実測で `contrast()` と `brightness()` を使っても
sRGB 0.83 以上まで持ち上げる必要があった）。
そのため「透明」は、
  - 画面に注意書きを出す（`glass_clear_note`）
  - 背後を強くぼかし、文字に縁取りを付けて輪郭だけは残す
  - e2e ではコントラストではなく「地を置いていないこと・縁取りが付いていること」を固定する
という扱いにしている。数値の検査（下記）の対象は オフ／うすく／しっかり の3段階。

**ふだんのコントラスト実測（`LOW_CONTRAST`）はガラスを検査できない。**
背景は `position: fixed` の別レイヤーで文字の祖先ではないため、半透明のカードを
重ねても「地は不透明な地の色」として計算される。そのため e2e に
「ガラスの下に最悪の色を敷いても読める」（明暗2×3段階×黒白）を別に置いている。
飾りを外し、地を黒と白で塗ってから測る。**塗った直後に測ってはいけない**
（地の色には200msの遷移が掛かっており、まだ元の色のままの値で通ってしまう。実測で空振りした）。
...
## 学習データの持ち出し（ファイル／Googleドライブ）

学習の記録は端末の localStorage にしか無いので、持ち出す手段を2つ用意している。
**どちらも残す**こと。ドライブが使えない場面（鍵を設定していない配布・ログインを断った・
iOSでホーム画面から開くとポップアップが戻らない）でも、ファイルで引っ越せる必要がある。

- 中身の組み立てと書き戻しは `src/backupPayload.ts` に**一本化**する
  （`buildBackupPayload` / `applyBackupPayload` / `isBackupPayload`）。
  画面ごとに集め直すと、片方にだけ新しいキーが入る・片方だけ検査が甘い、というずれが出る。
- ドライブの置き場所は `appDataFolder`（アプリ専用の隠しフォルダ）。求める権限は
  `drive.appdata` だけで、利用者の他のファイルは読めない。
  **学習データはこちらのサーバーを通さない**（プライバシーポリシーの約束を保つための選択）。
- **自動では同期しない。** 「ドライブへ保存」「ドライブから復元」を利用者が選ぶ。
...
テストは `tests/driveBackup.test.ts`（置き場所・権限・壊れた中身・許可切れ）と
